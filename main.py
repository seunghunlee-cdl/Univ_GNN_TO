...
count = np.zeros((len(mesh.coordinates()), 1))
...
while loop < maxiter:
    ...
    if(loop<Ni+Wi) or (divmod(max(loop-Ni-Wi,1),Nf)[1]==0):
        ...
        ## MMA parameters
        if xsolv == 0:
            phih.vector()[:] = oc(phih, volfrac, dc.vector()[:], dv, mesh)
        elif xsolv ==1:
            tic = time()
            mu0 = 1.0
            mu1 = 1.0
        ...
        with torch.no_grad():
            ...
            # dc_pred.vector()[dc_pred.vector()[:]>0] = 0
            # plot(dc_pred)
        ...
        ## MMA parameters
        if xsolv == 0:
            phih.vector()[:] = oc(phih, volfrac, dc_pred.vector()[:], dv, mesh)
        elif xsolv ==1:
            tic = time()
            mu0 = 1.0
            mu1 = 1.0
